...
# 3. 사용자 입력으로 메뉴와 가격을 입력 받아 menu변수에 자료를
# 추가 할 수 있도록 하시오.(동일한 메뉴는 가격만 변경)
menu = {'칼국수':6000,'비빔밥':5500,'돼지국밥':7000,'돈까스':7000,'김밥':2000,'라면':2500}
while True:
    name=input("메뉴를 입력하세요 : ")
...
#  
#  1) 중복불가
se = {2, 3, 4, 3, 1}  # 숫자인 경우에는 정렬되어 표시됨.
print(len(se))
print(se)
...
print(st.union(st2))            # {'h', 'w', 'o', 'r', 'l', 'e', 'd'}
...
# Python 문자열 실습 문제
# 문제 1. input() 함수로 이름과 나이 값을 입력 받을 때 한번에 입력
#        받아 처리하고 출력하는 코드를 작성하시오
#        예 ) 이름과 나이를 입력하세요 : 홍길동 18
#        => 이름 : 홍길동, 나이 : 18살
a = input("이름과 나이를 입력하세요 : ")
name, age = a.split()
print(f"이름 : {name}, 나이 : {age}")

# 문제 2. input() 함수로 입력 받은 수의 더하기, 빼기, 곱하기, 나누기의 
#        간단한 수식을 처리할 수 있는 코드를 작성하시오
#        예 ) 계산식을 입력하세요 : 5+5
#             계산 결과 : 10
num = input("계산식을 입력하세요 : ")
a,b,c = num
if b == '+':
    print("계산 결과 : ",int(a)+int(c))
elif b == '-':
    print("계산 결과 : ",int(a)-int(c))
elif b == '*':
    print("계산 결과 : ",int(a)*int(c))
elif b == '/':
    print("계산 결과 : ",float(a)/float(c))    
...
while True:
    ...
    if '+' in calc:
        ...
        num1 = int(num1)
        num2 = int(num2)
        print("계산 결과 : {:.2f}".format(num1/num2))
    else:
...
# 논리적인 절차를 의미하는 로직을 기초로 삼고 있기 때문에 이를 통해서 문제를
# 해결하는 것을 말함
#
#
#  알고리즘의 만족 조건
...
### 숙제 2장 4번과 3,4장의 연습문제 풀어서 다음주 월요일까지 메일로
# kdw335552@nate.com